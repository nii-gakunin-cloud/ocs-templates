...
Period: TypeAlias = (
    DailyPeriod | WeeklyPeriod | MonthlyPeriod | YearlyPeriod | SpecificDatePeriod
)
...
class SwarmSchedule(TypedDict):
    day_of_week: int | str
    hour: int
    minute: int


class SwarmScheduleEntry(SwarmNodeStatus):
    schedule: SwarmNodeStatus


SwarmScheduleDefinition: TypeAlias = list[SwarmScheduleEntry]
...
def _to_schedule(txt: str) -> Schedule:
    ...
    if m := WEEKLY_PATTERN.match(txt):
        return WeeklySchedule(
            day_of_week=WEEKDAY.to_weekday(m.group(1)),
            hour=int(m.group(2)),
...
def _to_period(begin_txt: str, end_txt: str):
    ...
    if "month" in begin or "month" in end:
        if "month" not in begin or "month" not in end:
            ...
            raise ValueError(msg)
        return YearlyPeriod(
            begin=cast(YearlySchedule, begin), end=cast(YearlySchedule, end)
        )
    if "day" in begin or "day" in end:
        ...
        return MonthlyPeriod(
            begin=cast(MonthlySchedule, begin), end=cast(MonthlySchedule, end)
        )
    if "day_of_week" in begin or "day_of_week" in end:
        if "day_of_week" not in begin or "day_of_week" not in end:
            ...
            raise ValueError(msg)
        return WeeklyPeriod(
            begin=cast(WeeklySchedule, begin), end=cast(WeeklySchedule, end)
        )
...
def get_schedule(params: dict) -> list[OcsNodeSchedule]:
    ...
    for item in params["schedule_list"]:
        ...
        schedule = OcsNodeSchedule(
            node_count=int(nodes_txt), period=_to_period(begin_txt, end_txt)
        )
...
def _get_default_node_count(ugroup_name: str, params: dict) -> int:
    if "schedule_default_nodes" in params:
        return int(params["schedule_default_nodes"])
    if (value := load_group_var(ugroup_name, "worker_nodes")) is not None:
        return int(value)
    return 0


def _get_max_nodes(ugroup_name: str) -> int:
    value = load_group_var(ugroup_name, "vcnode_all_ipaddress")
    if value is not None:
        return len(value)
    value = load_group_var(ugroup_name, "worker_nodes")
    if value is not None:
        return int(value)
    msg = f"vcnode_all_ipaddress is not defined in {ugroup_name}"
    raise ValueError(msg)
...
def _get_default_params(ugroup_name: str, params: dict) -> NodeDefaultParams:
    return {
        ...
        "max_node_count": _get_max_nodes(ugroup_name),
...
def _get_schedule_definition(
    ugroup_name: str, params: dict | None = None
...
def description_schedule(
    ...
    with TemporaryDirectory() as work_dir:
        ...
        if "TZ" in os.environ:
            del os.environ["TZ"]
        subprocess.run(
            [CMD_SCHEDULE_UTILS, "-D", "-c", str(cfg_path), "--desc-type", desc_type],
            check=True,
        )


def _get_swarm_schedule(params: dict) -> SwarmScheduleDefinition:
    schedules = params.get("vcnode_schedule")
    if schedules is None or not isinstance(schedules, list):
        msg = "vcnode_schedule is not defined"
        raise ValueError(msg)
    cfg = []
    for schedule in schedules:
        ent = schedule["availability"].copy()
        ent["schedule"] = schedule["schedule"]
        cfg.append(ent)
    return cfg


def _get_swarm_down_type(params: dict) -> str:
    gvars = load_group_vars(params["ugroup_name"])
    match gvars.get("schedule"):
        case "scale-out":
            return "deleted"
        case "power-ctl":
            return "power_down"
        case _:
            msg = "schedule is not defined"
            raise ValueError(msg)


def description_swarm_schedule(
    ugroup_name: str, params: dict | None = None, *, desc_type: str = "all"
) -> None:
    if params is None:
        params = load_group_vars(ugroup_name)
    with TemporaryDirectory() as work_dir:
        cfg_path = Path(work_dir) / "schedule.json"
        cfg = _get_swarm_schedule(params)
        with cfg_path.open(mode="w", encoding="utf-8") as f:
            json.dump(cfg, f)
        if "TZ" in os.environ:
            del os.environ["TZ"]
        down_type = _get_swarm_down_type(params)
        cmd = [
            CMD_SCHEDULE_UTILS,
            "-D",
            "-c",
        ...
        if len(err := ret.stderr.decode("utf-8").splitlines()) > 0:
            for line in err:
                print(line, file=sys.stderr)
        lines = ret.stdout.decode("utf-8").splitlines()
        for line in lines:
            print(line)
        if len(err) > 0 or len(lines) == 3:
            match down_type:
                case "power_down":
                    print("ノード停止時は電源をオフにする")
                case "deleted":
                    print("ノード停止時はノードを削除する")
                case _:
                    pass
...
def _schedule_to_txt(schedule: Schedule, lang: str) -> str:
    ...
    if "day_of_week" in schedule:
        ...
        return (
            f"{WEEKDAY.to_text(ws['day_of_week'], lang)} {ws['hour']}:{ws['minute']:02}"
        )
...
def convert_to_ocs(
    ...
    with TemporaryDirectory() as work_dir:
        cfg_path = Path(work_dir) / "schedule.json"
        ocs_cfg_path = Path(work_dir) / "schedule-ocs.json"
        cfg = _get_swarm_schedule(params)
        with cfg_path.open(mode="w", encoding="utf-8") as f:
            json.dump(cfg, f)
        if "TZ" in os.environ:
        ...
        with ocs_cfg_path.open(mode="r", encoding="utf-8") as f:
            return json.load(f)
