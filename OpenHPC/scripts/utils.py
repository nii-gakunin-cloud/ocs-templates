...
def check_parameter_master_ipaddress(value, params, kwargs):
    _check_ipv4_format(value, '#マスターノードのIPアドレスとホスト名')
    if (params['vc_provider'] != 'onpremisses' and
...
def check_parameter_compute_etc_hosts(value, params, kwargs):
    ...
    for ipaddr in value.keys():
        _check_ipv4_format(ipaddr, '#計算ノードのIPアドレスとホスト名')

    if params['vc_provider'] == 'onpremisses':
        return
...
def spec_env_etc_hosts(gvars):
    etc_hosts_kv = copy.copy(gvars["compute_etc_hosts"])
    etc_hosts_kv[gvars["master_ipaddress"]] = gvars["master_hostname"]
    return ",".join([f"{ip}:{host}" for ip, host in etc_hosts_kv.items()])
