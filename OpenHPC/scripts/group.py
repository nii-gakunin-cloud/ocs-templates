...
def store_group_vars(target_group, gvars, work_dir=os.getcwd()):
    ...
    with open(path, 'w') as f:
        yaml.dump(gvars, f, default_flow_style=False)
...
def ansible_vault_encrypt(value):
    ...
    if isinstance(output, six.binary_type):
        ...
        f = StringIO(output)
    encrypt_value = yaml.load(f, Loader=yaml.SafeLoader)
    f.close()
...
def update_group_vars(_target_group, _encrypt=False, **args):
    if _encrypt:
        args = encrypt_args(**args)
    gvars = _load_group_vars(_target_group)
    gvars.update(args)
    store_group_vars(_target_group, gvars)
