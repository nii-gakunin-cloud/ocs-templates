...
def update_inventory_yml(
    ...
    if inventory_path.exists():
        ...
        inventory = (
            _merge_dict(new_value, current_value) if current_value is not None else new_value
        )
...
def setup_ansible_cfg(inventory_path: PathOptType = None, backup: str | None = ".bak") -> Path:
    if inventory_path is None:
        inventory_path = Path("inventory.yml")
