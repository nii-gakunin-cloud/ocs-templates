...
def _set_init_passwd(mdx, vm, priv_key_path, passwd, user='mdxuser'):
    info = mdx.get_vm_info(vm)
    addr = info["service_networks"][0]["ipv4_address"][0]
    subprocess.run([f'{os.path.dirname(__file__)}/init_mdx_passwd.exp', user, addr, priv_key_path, passwd],
                    check=True)
...
def _change_ipaddr(mdx, vm, newaddr, priv_key_path, user='mdxuser'):
    ...
    info = mdx.get_vm_info(vm)
    addr = info["service_networks"][0]["ipv4_address"][0]
    subprocess.run(['scp'] + sshopts + [f'{os.path.dirname(__file__)}/mdx_change_addr.sh', f'{user}@{addr}:'], check=True)
...
class _MdxVcpInitializer(object):
    ...
    def __call__(self, addr):
        ...
        subprocess.run(['scp', '-p'] + sshopts + [f'{os.path.dirname(__file__)}/init_mdx_node.sh', f'{self.user}@{addr}:'], check=True)
